...
prot   <- fread(paste0(root,"results/cross_compartment_proteome_overlap.csv"))
...
B <- data.table(
  ...
  `ΔSD (q)` = c(
    ...
    cell(getb("VamsPostnatalInfant","vam_1005523","pn12")$est, getb("VamsPostnatalInfant","vam_1005523","pn12")$q)))
...
C <- data.table(
  ...
  `Milk, ΔSD (q)`          = mapply(cell, psel$milk_est, psel$milk_pval),
  `Maternal blood, ΔSD (q)`= mapply(cell, psel$blood_est, psel$blood_pval))
...
writeLines(c("### Panel A. Metabolites increased in maternal AND infant blood (VAMS, 5-6 mo.)","",
             md(A),"","### Panel B. Octenoylcarnitine across compartments","",
             md(B),"","### Panel C. Selenoproteins concordant in milk & maternal blood","",
             md(C)), out)
