# =============================================================================
# src/3 visualizations/7-microbiome_plots.R
#
...
# Writes: figures/forest_plot_microbiome_diversity.png
#         figures/volcano_plot_microbiome.png
#
...
d <- readRDS(paste0(here::here(),"/results/microbiome_intervention_effects_results.RDS"))
d_diversity <- readRDS(paste0(here::here(),"/results/microbiome_diversity_intervention_effects_results.RDS"))
head(d)
...
p_microbiome_plot2 <- plot_imic_volcano(res %>% filter(grepl("Mumta",studytime)))
...
ggsave(p_microbiome_plot, file=paste0(here::here(),"/figures/volcano_plot_microbiome.png"), width=18, height=10)
...
p_diversity <-   ggplot(res_diversity, aes(x=studytime, y=est, color=study, shape=factor(sigFDR))) + geom_point() +
  ...
  theme(strip.background = element_blank(),
        ...
        legend.position="none") + xlab("Study and timepoint") + ylab("Z-score difference")
p_diversity <-   ggplot(res_diversity, aes(x=studytime, y=est)) + geom_point() +
    geom_linerange(aes(ymin=cil, ymax=ciu)) +
    geom_hline(yintercept = 0, linetype="dashed") +
    coord_flip() +
    facet_wrap(~label_f, scale="free") +
...
ggsave(p_diversity, file=paste0(here::here(),"/figures/forest_plot_microbiome_diversity.png"), width=9, height=4)
